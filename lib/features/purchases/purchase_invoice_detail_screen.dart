...
class _PurchaseInvoiceDetailScreenState extends ConsumerState<PurchaseInvoiceDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final lData = await svc.client.from('purchaseinvoicelines').select('*').eq('purchaseinvoiceid', inv.id);
      final ids = <int>{};
      for (final j in lData) {
        final rid = j['rawmaterialid'];
        if (rid != null) ids.add(rid is int ? rid : int.tryParse(rid.toString()) ?? 0);
      }
      ids.remove(0);
      final Map<int, String> rmNames = {};
      if (ids.isNotEmpty && cid != null) {
        try {
          final rData = await svc.client.from('rawmaterials').select('id,designation').eq('companyid', cid.toString());
          for (final r in rData) {
            final rId = r['id'];
            if (rId != null && ids.contains(rId is int ? rId : int.tryParse(rId.toString()))) {
              rmNames[rId is int ? rId : (int.tryParse(rId.toString()) ?? 0)] = r['designation'] ?? '';
            }
          }
        } catch (_) {}
      }
      final lines = lData.map((j) {
        ...
        return PurchaseInvoiceLine(
          id: j['id']??0, purchaseInvoiceId: j['purchaseinvoiceid']??0, rawMaterialId: rIdInt,
          ...
          rawMaterialName: rmNames[rIdInt],
        );
      }).toList();
      double ht = 0;
